...
CREATE TABLE IF NOT EXISTS configuracion_so (
  ...
  actividad_vulnerable VARCHAR(100) NOT NULL, -- Art. 17
  ...
  umbral_operacion_relevante DECIMAL(15,2) DEFAULT 17500.00, -- USD
  umbral_efectivo DECIMAL(15,2) DEFAULT 7500.00, -- USD
...
CREATE TABLE IF NOT EXISTS clientes (
  ...
  actividad_economica_sat VARCHAR(10), -- Código SAT
  ...
  validacion_sat_situacion VARCHAR(50), -- Activo, Suspendido, Cancelado
...
CREATE TABLE IF NOT EXISTS cliente_documentos (
  ...
  ocr_data JSONB,
  datos_extraidos JSONB, -- Nombre, CURP, RFC, etc. extraídos automáticamente
  ...
  vigente BOOLEAN GENERATED ALWAYS AS (
    CASE 
      WHEN fecha_vencimiento IS NULL THEN true
...
CREATE TABLE IF NOT EXISTS busquedas_listas (
  ...
  metodo VARCHAR(50), -- 'exacto', 'fuzzy', 'api'
  
  -- Resultados
  coincidencias_encontradas INTEGER DEFAULT 0,
  resultado JSONB, -- Array de coincidencias con detalles
  
  -- API Externa (si aplica)
  api_provider VARCHAR(100), -- 'WorldCheck', 'Comply Advantage', etc.
...
CREATE TABLE IF NOT EXISTS reportes_uif (
  ...
  tipo_reporte VARCHAR(50) NOT NULL CHECK (tipo_reporte IN (
    'mensual_relevantes',      -- Art. 17 - Operaciones Relevantes
    'aviso_24hrs',             -- Art. 17 - Operaciones Preocupantes
    'informe_ausencia',        -- Si no hubo operaciones relevantes
    'beneficiario_controlador' -- Anual
  )),
  
  -- Periodo
  periodo_inicio DATE NOT NULL,
  periodo_fin DATE NOT NULL,
  mes_reporte INTEGER, -- 1-12
  anio_reporte INTEGER,
  
  -- Operaciones Incluidas
  num_operaciones INTEGER DEFAULT 0,
  monto_total DECIMAL(15,2) DEFAULT 0,
  operaciones_ids UUID[], -- Array de operacion_id
  ...
  numero_operacion_uif VARCHAR(100), -- Folio UIF
...
-- ✅ TABLA 7: Operaciones Transaccionales (después de analisis_ml y reportes_uif)
CREATE TABLE IF NOT EXISTS operaciones (
  ...
  monto_usd DECIMAL(15,2), -- Convertido
  monto_umas DECIMAL(10,2), -- Para umbrales LFPIORPI
  
  -- Tipo de Operación
  tipo_operacion VARCHAR(100) NOT NULL,
  clasificacion_operacion VARCHAR(50), -- 'deposito', 'retiro', 'transferencia', etc.
  ...
  -- Guardrails LFPIORPI
  supera_umbral_17500 BOOLEAN GENERATED ALWAYS AS (monto_usd >= 17500) STORED,
